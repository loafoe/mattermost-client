...
const tlsverify = !(process.env.MATTERMOST_TLS_VERIFY || '').match(
	/^false|0|no|off$/i
);
const useTLS = !(process.env.MATTERMOST_USE_TLS || '').match(
	/^false|0|no|off$/i
);
...
class Client extends EventEmitter {
	constructor(host, group, options) {
		...
		this._pingInterval =
			this.options.pingInterval != null
				? this.options.pingInterval
				: defaultPingInterval;

		this.autoReconnect =
			this.options.autoReconnect != null ? this.options.autoReconnect : true;

		this.httpProxy =
			this.options.httpProxy != null ? this.options.httpProxy : false;
	...
	login(email, password, mfaToken) {
		...
		return this._apiCall(
			'POST',
			usersRoute + '/login',
			{ login_id: this.email, password: this.password, token: this.mfaToken },
			this._onLogin
		);
	}
		...
		if (data) {
			if (!data.id) {
				...
				this.socketUrl =
					(useTLS ? 'wss://' : 'ws://') +
					this.host +
					(useTLS && this.options.wssPort != null
						? `:${this.options.wssPort}`
						: this.options.httpPort != null
	...
	_onChannels(data, headers, params) {
		if (data && !data.error) {
			...
			this.logger.info(
				`Found ${Object.keys(data).length} subscribed channels.`
			);
			return this.emit('channelsLoaded', data);
		} else {
			this.logger.error(
				`Failed to get subscribed channels list from server: ${data.error}`
			);
		...
		this.ws.on('open', () => {
			...
			return (this._pongTimeout = setInterval(() => {
				...
				if (
					...
					this.logger.error(
						'Last pong is too old: %d',
						(Date.now() - this._lastPong) / 1000
					);
					this.authenticated = false;
					this.connected = false;
					return this.reconnect();
	...
	customMessage(postData, channelID) {
		...
		return this._apiCall('POST', '/posts', postData, (data, header) => {
			...
			if ((chunks != null ? chunks.length : undefined) > 0) {
				this.logger.debug(
					`Recursively posting remainder of customMessage: (${chunks.length})`
				);
	...
	findChannelByName(name) {
		for (let c in this.channels) {
			if (
				this.channels[c].name === name ||
				this.channels[c].display_name === name
			) {
	...
	_chunkMessage(msg) {
		...
		const message_length = new TextEncoder.TextEncoder('utf-8').encode(msg)
			.length;
	...
	postMessage(msg, channelID) {
		...
		return this._apiCall('POST', '/posts', postData, (data, header) => {
			...
			if ((chunks != null ? chunks.length : undefined) > 0) {
				msg = chunks.join();
				this.logger.debug(
					`Recursively posting remainder of message: (${
						chunks != null ? chunks.length : undefined
					})`
				);
	...
	_apiCall(method, path, params, callback, callback_params) {
		...
		const options = {
			uri:
				(useTLS ? 'https://' : 'http://') +
				this.host +
				(this.options.httpPort != null ? `:${this.options.httpPort}` : '') +
				apiPrefix +
				path,
			method,
			json: params,
			...
			headers: {
				...
				'Content-Length': new TextEncoder.TextEncoder('utf-8').encode(post_data)
					.length
			}
		};
				...
				if (callback) {
					if (res.statusCode === 200 || res.statusCode === 201) {
						...
						return callback(
							{
								id: null,
								error: `API response: ${res.statusCode} ${JSON.stringify(
									value
								)}`
							},
							res.headers,
